...
def logg():
    ...
    if user in user_list:
        if passw in pass_list:
            l2.configure(text='Successfully Logged In!')
            screen2.destroy()
            user_dashboard()
        else:
            l2.configure(text='Invalid Password!',fg='red')
    else:
         l2.configure(text='User Not Found!',fg='red')
def bac():
    screen2.destroy()
    main_screen()
def login():
...
def submit():
    ...
    file.close()
    if user=="":
        l1.configure(text='Invalid Username!',fg='red')
    elif passw!=cpassw or passw=="" or cpassw=="":
        l1.configure(text='Invalid Password!',fg='red')
    elif user in d:
        l1.configure(text='User Already Exists!',fg='red')
    else:
        f= open('userinfo.txt','rb+')
        d.update(d1)
        f.seek(0)
        pickle.dump(d,f)
        f.close()
        l1.configure(text='Registered Please Log In!',fg='green')
        ent1.delete(0,END)
        ent2.delete(0, END)
        ent3.delete(0, END)
def back():
